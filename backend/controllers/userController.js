...
const userRegister = async (req, res) =>{
     ...
     try {
        ...
        if(user){
            return res.json({success: false, message:"user alredy exists, Try login!"})
        }
        ...
        return res.json({success: true,message: "resgistered Successfully", token, savedUser});
...
const adminLogin = async (req, res) => {
    try {
        ...
        if(email === process.env.ADMIN_EMAIL && password === process.env.ADMIN_PASSWORD){
            const token = jwt.sign({email: email, password:password}, process.env.JWT_SECRET, {expiresIn: "7d"});
